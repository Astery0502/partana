...
clear;
%% Set path of ARD
path('/share/home/guoyang1/apps/LoRD/matlab',path); % !!! Use your own path of LoRD

%% Load Data
DIM = [100,400,200]; % [n1,n2,n3]
Precision = 'double';
LIM = [-0.25,0.25,-1,1,0.3,1.3]; % [xmin,xmax,ymin,ymax,zmin,zmax]
[B1,x1,x2,x3] = Tool_LoadData_Bin('../data/b1.bin',DIM,Precision,LIM);
[B2,~,~,~] = Tool_LoadData_Bin('../data/b2.bin',DIM,Precision,LIM);
[B3,~,~,~] = Tool_LoadData_Bin('../data/b3.bin',DIM,Precision,LIM);
...
Parameters.OutputDir = '../data/';
Parameters.OutputLabel = 'asy';
...
Parameters.ARD_ScalarThreshold = 5;    % Threshold of DataThres
...
set(axes1,'XLim',[-3,3],'YLim',[-3,3],'ZLim',[0,6],...
...
R0_O = [0 0 1.0] % Position of sphere center
Radius_O = 0.1; % Radius of sphere
n_sample_O = 50; % Number of initial samplings
LineConf.Len = 10000;
LineConf.Color = 'k'; % Field-line color
LineConf.Style = '-'; % Field-line style
LineConf.Width = 0.01; % Field-line width
